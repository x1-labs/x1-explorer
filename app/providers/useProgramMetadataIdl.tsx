...
export function useProgramMetadataIdl(programAddress: string, url: string, cluster: Cluster) {
    const { data } = useSWRImmutable(`program-metadata-idl-${programAddress}-${url}`, async () => {
        if (!PMP_IDL_ENABLED) {
            return null;
        }
